...
class CodeGenerator {
    public:
        typedef std::unordered_map<Location*, std::list<Location*>>
            InterferenceGraph;
    ...
    private:
        ...
        Hashtable<int> *labelTable; // Label position table
        std::unordered_set<int> *succ; // Successor table
        std::vector<Location*> *in, *out, *kill; // IN, OUT, KILL set

    private:
        void PopulateLabelTable();
        void MarkSuccessor();
        void FuncLivenessAnalysis(int start, int end);
        void LivenessAnalysis();
        InterferenceGraph *BuildGraph(int start, int end);
        void RegisterAlloc(InterferenceGraph *graph, int start,
                           int end);
        void GraphColoring();
        void PopulateRegMap();
        void Optimise();
